...
const CreatePackage string = `INSERT INTO beanpay_package_info(account_id,spkg_id,pkg_name,total_capacity,total_remain,capacity_daily,
...
const GetPackageBySPKG string = `select t.pkg_id,t.total_remain from beanpay_package_info t 
where t.spkg_id=@spkg_id and t.account_id=@account_id`

//ChangePackage 服务数量变更
const ChangePackage string = `update beanpay_package_info t 
...
//ExistsPackageRecord 查询服务包变动记录是否已存在
const ExistsPackageRecord string = `select count(0) from beanpay_package_record t where t.trade_no=@trade_no and
 t.pkg_id=@pkg_id`
...
const QueryPackageRecord string = `select t.* from beanpay_package_record t 
