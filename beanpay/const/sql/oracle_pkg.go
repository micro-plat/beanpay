...
const AddPackageRecord string = `insert into beanpay_package_record(record_id,pkg_id,account_id,trade_no,change_type,num,remain,create_time)
select seq_package_record_id.nextval,t.pkg_id,t.account_id,@trade_no,@tp,@capacity,t.total_remain,sysdate from beanpay_package_info t where t.pkg_id=@pkg_id`
